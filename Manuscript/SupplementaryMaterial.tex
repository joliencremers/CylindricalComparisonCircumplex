...
\indent We use the following loglikelihoods for the computation of the
PLSL in the teacher data:

\begin{itemize}
\item For the modified CL-PN model:

$$l(y) = \log(1) - \log(\sqrt{2\pi\sigma^2}) + \sum(\hat{y}_i-(\gamma_0 + \gamma_{cos}\cos(\theta_i)r_i +
\gamma_{sin}\sin(\theta_i)r_i + \gamma_1\text{SE}_i))^2/2\sigma^2$$

$$l(\theta) = \log(1) - \log(2\pi) + \sum -0.5\hat{\mu}_i^2 - 0.5(r_i^2 - 2r_iu_i^t\hat{\mu}_i)$$
...
$$l(y) = \log\alpha + \sum\log h_i^\alpha + \sum\log y_i^{\alpha - 1} - \sum(h_iy_i)^\alpha$$
...
$$l(\theta) = \log(1) - \sum\log 2\pi I_0(c_i) + \sum\log\{1 + \lambda \sin(\theta_i - \hat{\theta}_i\} + \sum c_i\cos(\theta_i - \hat{\theta}_i)$$

where $c_i = y_i^{\alpha}\exp(\hat{y}_i)^{\alpha}\tanh\kappa$, and $I_{0}$ is a Bessel function of order $0$.
